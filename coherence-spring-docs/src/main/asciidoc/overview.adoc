...
IMPORTANT: We are referencing older versions of the Spring Framework in pom.xml because official updates for out-of-support versions are not freely available. The current list of supported versions can be found at https://github.com/spring-projects/spring-framework/wiki/Spring-Framework-Versions[Spring Framework Versions]. Access to patches, security fixes, and maintenance for these versions typically requires a commercial support subscription from VMware. As such, we rely on existing versions for continuity and compatibility in the absence of publicly available updates. If you have access to newer, supported dependencies, you can exclude unsupported dependencies from com.oracle.coherence.spring as described in the https://maven.apache.org/guides/introduction/introduction-to-optional-and-excludes-dependencies.html[Optional Dependencies and Dependency Exclusions] document.
