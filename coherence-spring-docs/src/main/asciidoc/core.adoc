...
In order to provide a better user-experience around the dependency injection of maps, Coherence Spring introduces its
own annotation. Using the `@NamedCache` annotation, you can inject any Coherence maps in any situation including constructors.

Furthermore, `@NamedCache` also gives you some added conveniences such as the ability to specify the name of the cache or
the name of the Coherence session as part of the annotation. E.g., the above example can be simplified to:

.Example of using the @NamedCache annotation
[source,java]
----
    @NamedCache
    private NamedCache numbers;                    <1>

    @NamedCache("numbers")                         <2>
    private NamedCache namedCache;
----
<1> If not specified, the name of the field will be used to determine the cache name
<2> Alternatively, you can specify the name of the cache using the `@Name` annotation
...
<3> Due to Spring limitations regarding the injection of Maps, we use the `@NamedCache` annotation to inject the `NamedMap`,
