...
IMPORTANT: Coherence Spring requires as a minimum version *Coherence CE* `22.06`. However, currently there is no
...
[IMPORTANT]
====
To avoid deadlocks where a cache bean requires another bean to be injected in its
configuration, which will happen on a different thread to the main Spring thread, all
cache beans injected by `@CoherenceCache`, `@CoherenceMap`, `@CoherenceAsyncCache`, and `@CoherenceAsyncMap` annotations
will be Spring lazy dynamic proxies.
====
...
link:{coherence-api}com/tangosol/net/events/partition/cache/EntryEvent.html[Entry Events] represent operations for 
inserting, removing, and updating entries in a cache.
...
Precommit entry events (INSERTING, REMOVING, and UPDATING)
are raised before the operation is performed to allow modification to an entry. The following holds true when
modifying entries:
...
Postcommit entry events (INSERTED, REMOVED, and UPDATED) are raised after an operation is performed and in the same
order as the events occurred. Postcommit events indicate that an entry is no longer able to be modified. Event
interceptors for postcommit entry events are asynchronously processed.

To subscribe to `EntryEvent` simply create a Spring bean with a listener method annotated with
link:{coherence-spring-api}com/oracle/coherence/spring/event/CoherenceEventListener.html[@CoherenceEventListener].
