...
To create a topic Publisher that sends messages you can simply define an interface that is annotated with link:{coherence-spring-api}com/oracle/coherence/spring/annotation/CoherencePublisher.html[@CoherencePublisher]. Also configuration class has to be annotated with link:{coherence-spring-api}com/oracle/coherence/spring/annotation/CoherencePublisherScan.html[@CoherencePublisherScan] annotation in order to specify base packages for scanning for `@CoherencePublisher` annotated interfaces.
