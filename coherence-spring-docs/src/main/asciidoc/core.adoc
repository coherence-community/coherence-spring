...
applications using the https://github.com/coherence-community/coherence-spring[Coherence Spring project]. The sample
has a simple class called `Person` annotated with JPA annotations:
...
----
include::{include-source-code-samples-cachestore}controller/PersonController.java[tag=code]
----
...
----
java -jar samples/cachestore-demo/target/coherence-spring-cachestore-demo-{coherence-spring-version}.jar
----
