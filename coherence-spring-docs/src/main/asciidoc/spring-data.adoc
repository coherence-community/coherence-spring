...
(among other things) to transfer over network just requested properties.
...
For closed projections Coherence can optimize the query execution as all attributes that are
needed to back the projection proxy are already known so EntryProcessor can be used to fetch just required data.
...
More flexible option is to implement the custom logic in a Spring bean:
