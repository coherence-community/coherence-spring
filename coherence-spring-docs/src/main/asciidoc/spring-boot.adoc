...
Once that is in place, you can take advantage of {spring-docs}html/integration.html#cache[Spring's Cache abstraction]
backed by Coherence and use Spring's annotations such as `@Cacheable`, `@CacheEvict`, `@CachePut` etc.

[NOTE]
====
When learning about Coherence's Spring cache abstraction support, please also familiarize yourself with the
following resources:
