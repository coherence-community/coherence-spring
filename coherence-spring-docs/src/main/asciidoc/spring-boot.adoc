...
NOTE: Under the covers Coherence Spring takes advantage of the Coherence Bootstrap API. For more information, check out
the respective chapter on the Bootstrap API in the
{coherence-docs}develop-applications/starting-and-stopping-cluster-members.html#GUID-A74280C1-AB23-48DB-8C0D-E494230ABFCA[Coherence reference guide].
...
When using Coherence Spring Boot, you may want to verify that only the auto-configuration that you actually need
is enabled. For example, Coherence is shipped with `Gson`, and the mere presence of the Gson class may trigger
`GsonAutoConfiguration`.

To find out which auto-configuration is applied for your application, start your Spring Boot application with the `--debug`
argument. The auto-configuration report will be logged to the console.

Any auto-configuration that you do not want to be applied, can be disabled by specifying the `exclude` property on the
`@SpringBootApplication` annotation on your main application class. Just specify the name of the auto-configuration classes
that you do not want to be applied.

[source,java]
----
@SpringBootApplication(exclude = GsonAutoConfiguration.class)
public class MyCoherenceApplication {
	public static void main(String[] args) {
        SpringApplication.run(MyCoherenceApplication.class, args);
    }
}
----

Alternatively, you can also disable specific auto-configuration by setting the `spring.autoconfigure.exclude` property
in your `application.properties` or `application.yaml` file.
