...
Coherence Spring includes an link:{sources-root-samples}circuit-breaker-cache-demo[example application] that illustrates
...
However, this is not true for the retrieve of all books in this example.
...
Next, we will first shut down the Coherence server, and then we try to retrieve a single book with id `1`. As you see,
the JSON response is still the same (The book data is returned). However, when you look at the logs, you will see:
